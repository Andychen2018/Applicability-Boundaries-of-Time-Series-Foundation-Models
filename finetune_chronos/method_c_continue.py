#!/usr/bin/env python3
"""
方案C续：使用已训练的Chronos模型进行embedding提取和分类
"""
...
def continue_method_c_single_domain(domain, output_dir, context_length=2048):
    """
    继续方案C：使用已训练的模型进行embedding提取和分类
    """
    print(f"\n=== 继续处理 {domain} 域的方案C模型 ===")
...
def main():
    ...
    # 处理各个域的模型
    domains = ["ZhenDong"]  # 处理ZhenDong域
    
    for domain in domains:
        try:
            success = continue_method_c_single_domain(domain, output_dir)
            if success:
                print(f"{domain} 域方案C处理完成")
            else:
                print(f"{domain} 域方案C处理失败")
        except Exception as e:
