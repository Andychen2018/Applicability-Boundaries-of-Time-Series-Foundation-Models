#!/usr/bin/env python3
"""
方案C：All-class微调 + embedding特征多分类
用三类数据共同微调Chronos，然后抽取embedding特征进行三分类
"""
...
def train_method_c_single_domain(domain, output_dir, prediction_length=1024, context_length=2048):
    """
    训练单个域的方案C模型
    """
    print(f"\n=== 开始训练 {domain} 域的方案C模型 ===")
    ...
    # 创建输出目录
    domain_output_dir = os.path.join(output_dir, f"methodC_embed_all_ft/{domain}")
    ...
    # 1. 准备all-class训练数据（包含所有三类）
    print(f"All-class训练数据: {len(train_domain)} 条记录, {train_domain['item_id'].nunique()} 个序列")
    print("标签分布:", train_domain['label'].value_counts().to_dict())
    
    # 准备Chronos数据格式
    print("准备Chronos训练数据...")
    chronos_train_data = prepare_chronos_data(train_domain, prediction_length, context_length)
    
    if chronos_train_data is None:
        print(f"警告: {domain} 域没有足够长度的数据进行训练")
        return
...
def main():
    ...
    # 训练各个域的模型
    domains = ["ZhenDong"]  # 只训练ZhenDong域
    
    for domain in domains:
        try:
            result = train_method_c_single_domain(
                domain, output_dir, prediction_length, context_length
            )
            if result:
                print(f"{domain} 域方案C训练完成")
            else:
                print(f"{domain} 域方案C训练失败")
        except Exception as e:
